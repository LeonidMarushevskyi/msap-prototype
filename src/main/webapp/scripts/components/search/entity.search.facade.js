...
angular.module('msapApp')
    .factory('EntitySearchFacade', ['$log', '$injector', '$q', 'ParseLinks',
        function ($log, $injector, $q, ParseLinks) {
             ...
             *
             * Examples:
             *      CWSSearchFacade._searchEntity(ReferralSearch, stringQuery, pagination = { pageSize: $scope.PAGE_SIZE }).then(...);
             *      CWSSearchFacade._searchEntity(ReferralSearch, stringQuery, null, true, errorCallback).then(...);
             *
             ...
             * @param pagination
             * @param usePOST - todo: test it with pagination together
             * @param errorCallback
             * @return promise of the search service query() method or null if nothing to search
             */
            function _searchEntity(EntitySearchService, stringQuery, pagination, usePOST, errorCallback) {
                return _.isNil(stringQuery) || _.isEmpty(stringQuery.trim()) ? null :
                    (usePOST ? EntitySearchService.queryPost : EntitySearchService.query)(
            ...
            /*
             ...
             { '-': [ fn1: 'abc', fn2: 'def' ] }
                -> -(fn1:abc fn2:def)
             */
            function _processGroup(sign, searchExpr, reusableOptions) {
                return _([
                    (sign ? sign : '')
            ...
            function _buildStringQuery(searchExpr, reusableOptions) {
                if (_.isNil(searchExpr)) {
                    ...
                    if (_.isEmpty(searchExpr)) {
                        ...
                        //  -> (+fn1:abc -fn2:def)
                        return _processGroup('', searchExpr, reusableOptions);
            ...
            function _searchStringQuery(stringQuery, entityName, pagination, usePOST) {
                return createSearchResults(stringQuery, entityName,
                    _.isEmpty(stringQuery) ? []
                        : _searchEntity($injector.get(entityName + 'Search'), stringQuery, pagination, usePOST)
                );
            }
            ...
            function search(searchExpr, entityName, reusableOptions, pagination, usePOST) {
            ...
            function fetchList(searchExpr, entityName, reusableOptions, pagination, usePOST) {
                return search(searchExpr, entityName, reusableOptions, pagination, usePOST).then(function (result) {
                    return result.data;
                });
            }
